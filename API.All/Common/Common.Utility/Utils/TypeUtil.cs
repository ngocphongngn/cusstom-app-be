...
namespace Common.Utility.Utils
{
    ...
    public static class TypeUtil
    {
        ...
        /// </summary>
        public static Type GetTypeInList(Type listType)
        {
            if (IsList(listType))
            {
        ...
        private static ConcurrentDictionary<string, List<PropertyInfo>> _entityProperties = new ConcurrentDictionary<string, List<PropertyInfo>>();
        /// <summary>
        ...
        private static ConcurrentDictionary<string, Dictionary<string, Dictionary<PropertyInfo, Attribute>>> _modelAttributes = new ConcurrentDictionary<string, Dictionary<string, Dictionary<PropertyInfo, Attribute>>>();
        /// <summary>
        /// Lấy danh sách các thuộc tính theo attribute
        /// </summary>
        public static Dictionary<PropertyInfo, Attribute> GetPropertys<TAttribute>(Type entityType) where TAttribute : Attribute
        {
            if(entityType == null)
            {
                return null;
        ...
        private static ConcurrentDictionary<string, List<PropertyInfo>> _tableProperties = new ConcurrentDictionary<string, List<PropertyInfo>>();
        /// <summary>
        /// Lấy thuộc tính mapping với các trường dữ liệu trogn db
        /// </summary>
        public static List<PropertyInfo> GetTableProperty(Type type)
        {
            if(type == null)
            {
                return null;
